...
export const normalizePost = (entry) => ({
  slug: entry.id,
  ...entry.data,
  pubDate: isoDate(entry.data.pubDate),
  updatedDate: isoDate(entry.data.updatedDate),
...
export const posts = async () =>
  (await allPosts()).filter((post) => !post.unlisted);
...
export const sortedPosts = async () =>
  [...(await posts())].sort((a, b) => (a.pubDate < b.pubDate ? 1 : -1));
